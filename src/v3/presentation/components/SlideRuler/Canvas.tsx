...
type TouchType = React.TouchEvent<HTMLCanvasElement> | React.MouseEvent<HTMLCanvasElement>
...
export default function Canvas({
  ...
  function getCoordinate(e: unknown): number {
    const { pageX, pageY } = (e as React.TouchEvent<HTMLCanvasElement>).touches?.[0] ?? e
  ...
  function moveGradations(delta: number) {
    ...
    const increment = Math.sign(diff) * step
    let speed = Math.abs(diff)
