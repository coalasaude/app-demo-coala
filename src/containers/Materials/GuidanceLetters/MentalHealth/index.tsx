...
import { MentalHealthLettersConfig } from './config'

export const MentalHealthLetters = () => {
  const isSmallDevice = useMediaQuery('sm')
  return (
    <>
      <PageHeader
        title='Saúde mental'
        onBack={() => Router.push(NEW_ROUTES.AUTHENTICATED.MATERIALS.GUIDANCE_LETTERS.path)}
      />
      <GridWrapper>
        {MentalHealthLettersConfig.map(({ name, image, url, urlName }) => {
