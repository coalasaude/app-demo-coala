...
type DrawCanvas = Required<
  Omit<
    SlideRuleProps,
    'style' | 'showWarning' | 'cursor' | 'axis' | 'value' | 'gap' | 'height' | 'onChange' | 'width'
  > & {
...
const _drawVerticalLine = (ctx: CanvasRenderingContext2D, coordinate: number, style: MarkStyle) => {
...
const _drawLine = (ctx: CanvasRenderingContext2D, coordinate: number, style: MarkStyle) => {
...
const _applyNumberNumberStyle = (ctx: CanvasRenderingContext2D, numberStyle: NumberStyle) => {
...
const validatePointersValue = (
  ...
  const defaults = isXAxis ? DEFAULT_POINTER_X_AXIS_STYLES : DEFAULT_POINTER_Y_AXIS_STYLES
...
const drawCanvas = ({
  ...
  const lower = Math.round(min / step)
